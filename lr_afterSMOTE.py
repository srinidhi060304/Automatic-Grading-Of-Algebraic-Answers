...
# Now, you can use the best parameters found by grid search for Logistic Regression
best_lr = LogisticRegression(max_iter=1000, C=best_params['C'])

# Perform cross-validation and calculate evaluation metrics
accuracy = cross_val_score(best_lr, X_train, y_train, cv=kf, scoring='accuracy')
precision = cross_val_score(best_lr, X_train, y_train, cv=kf, scoring='precision_macro')
recall = cross_val_score(best_lr, X_train, y_train, cv=kf, scoring='recall_macro')
f1 = cross_val_score(best_lr, X_train, y_train, cv=kf, scoring='f1_macro')
...
testing_output_file = "predicted_testing_mathbert_lr.xlsx"
