...
data = pd.read_excel(r'D:\srinidhi\amrita\out of context\out of context\resampled_training_mathbert.xlsx')

# Convert DataFrame to NumPy arrays
X_train = data.iloc[:, :-1].values  # Features (embeddings)
y_train = data['Classification'].values  # Target
...
cv_accuracy = cross_val_score(best_knn, X_train, y_train, cv=5, scoring='accuracy')
cv_precision = cross_val_score(best_knn, X_train, y_train, cv=5, scoring='precision_weighted')
cv_recall = cross_val_score(best_knn, X_train, y_train, cv=5, scoring='recall_weighted')
cv_f1 = cross_val_score(best_knn, X_train, y_train, cv=5, scoring='f1_weighted')
...
testing_output_file = "predicted_testing_mathbert_knn.xlsx"
