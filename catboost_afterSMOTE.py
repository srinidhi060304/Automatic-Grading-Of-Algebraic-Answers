...
# Load the dataset
file_path = r'D:\srinidhi\amrita\out of context\out of context\resampled_training_mathbert.xlsx'
...
grid_search = GridSearchCV(estimator=catboost, param_grid=param_grid, cv=kf, scoring='accuracy')
...
# Now, you can use the best parameters found by grid search for CatBoost
best_catboost = CatBoostClassifier(**best_params, random_state=42, verbose=False)

# Fit the best CatBoost classifier with the entire training data
best_catboost.fit(X, y)

# Perform cross-validation and calculate evaluation metrics
accuracy = cross_val_score(best_catboost, X, y, cv=kf, scoring='accuracy')
precision = cross_val_score(best_catboost, X, y, cv=kf, scoring='precision_macro')
recall = cross_val_score(best_catboost, X, y, cv=kf, scoring='recall_macro')
f1 = cross_val_score(best_catboost, X, y, cv=kf, scoring='f1_macro')
...
testing_output_file = "predicted_testing_mathbert_catboost.xlsx"
