...
y = df['Classification'] - 1  # Adjust classes to start from 0
...
grid_search = GridSearchCV(estimator=xgb, param_grid=param_grid, cv=kf, scoring='accuracy')
...
# Now, you can use the best parameters found by grid search for XGBoost
best_xgb = XGBClassifier(**best_params, random_state=42)

# Perform cross-validation and calculate evaluation metrics
accuracy = cross_val_score(best_xgb, X, y, cv=kf, scoring='accuracy')
precision = cross_val_score(best_xgb, X, y, cv=kf, scoring='precision_macro')
recall = cross_val_score(best_xgb, X, y, cv=kf, scoring='recall_macro')
f1 = cross_val_score(best_xgb, X, y, cv=kf, scoring='f1_macro')
...
testing_output_file = "predicted_testing_mathbert_xgb.xlsx"
