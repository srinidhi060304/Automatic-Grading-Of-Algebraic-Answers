...
grid_search = GridSearchCV(estimator=mlp, param_grid=param_grid, cv=kf, scoring='accuracy')
...
# Now, you can use the best parameters found by grid search for MLP
best_mlp = MLPClassifier(**best_params, random_state=42)

# Fit the best MLP classifier with the entire training data
best_mlp.fit(X, y)

# Perform cross-validation and calculate evaluation metrics
accuracy = cross_val_score(best_mlp, X, y, cv=kf, scoring='accuracy')
precision = cross_val_score(best_mlp, X, y, cv=kf, scoring='precision_macro')
recall = cross_val_score(best_mlp, X, y, cv=kf, scoring='recall_macro')
f1 = cross_val_score(best_mlp, X, y, cv=kf, scoring='f1_macro')
...
testing_output_file = "predicted_testing_mathbert_mlp.xlsx"
